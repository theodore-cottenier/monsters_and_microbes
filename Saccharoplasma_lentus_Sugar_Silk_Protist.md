# Companion Profile: *Saccharoplasma lentus*

**Common name:** Sugar-Silk Protist  
**Role:** Protocist refiner for *Hydrosaltator amictus*

## Scientific summary

*Saccharoplasma lentus* is a slow-moving protocist that refines complex carbon compounds into syrupy, transportable sugars. It lives in the cooler folds of the host matrix, where its threadlike secretions connect harvester pockets to feeding cells.

- **Preferred zone:** 50-72 degrees C matrix folds
- **Form:** Translucent sacs with trailing silk-like filaments
- **Primary resource:** Sulfur-harvester byproducts and dissolved organics
- **Exchange:** Refined sugars and flexible transport threads for mineral nutrients

The Sugar-Silk Protist is famously unhurried: a single refinement cycle can take several minutes, which is considered “punctual” by vent standards.

*The expanded behavior and anatomy below are speculative worldbuilding for this fictional organism.*

## Detailed behavior

Sugar-Silk settles in cooler matrix folds and draws in dissolved organics and carbon-rich harvester byproducts. It encloses more complex material in digestive compartments, processes it over several minutes, and releases refined sugars in measured pulses. Its trailing threads retain these products along routes linking harvester pockets, feeding cells, and matrix gardeners.

Movement is a slow cycle of extending the leading edge, attaching, and drawing the sac forward. A productive site encourages longer threads and larger reserves; a depleted site prompts gradual relocation while usable trailing connections remain in place. Strong flow causes the sac to flatten and shorten exposed threads. Small buds separate when food is plentiful, each carrying enough stored carbon to begin its own unhurried refinement cycle.

## Detailed anatomy

**Sac and outer covering.** The translucent body is a flexible sac with a thin supporting pellicle beneath a slippery coat. The leading surface can form broad temporary lobes for crawling, while the trailing end narrows around the thread-producing region. Gentle folds allow the body to expand as food compartments fill.

**Digestive interior.** A distinct nuclear compartment lies among digestive vacuoles, storage droplets, and energy-producing organelles. Incoming material enters small vesicles that merge with larger processing compartments. Their membranes keep unfinished digestive products separate from the refined sugars held for export.

**Sugar reservoirs.** Clear internal pockets collect soluble products before release. Transport sites regulate their passage across the outer membrane, preventing every refinement cycle from emptying at once. Water-regulating compartments help maintain the sac's shape as the concentration of stored sugars changes.

**Silk secretion apparatus.** Rows of secretion vesicles feed narrow outlets at the trailing pole. The resulting hydrated polymer filaments adhere to gel and carry sugar-rich coatings along their surfaces. These are extracellular transport threads, rather than hollow blood vessels, and can be renewed where movement stretches or breaks them.

**Attachment and budding.** Temporary adhesive patches beneath the sac provide traction. A developing bud forms along a side fold, receiving living contents and reserves before a narrow connecting neck closes. Short initial threads help the daughter remain near a useful feeding pocket.

## Relationships in the wider food web

The following direct links appear in the [illustrated community food web](hydrosalt-cryptids/README.md). Exchange arrows run from provider to recipient; predation, parasitism, and infection run from attacker to target.

| Edge | From | To | Interaction |
| --- | --- | --- | --- |
| E01 | *Sulfuribacillus thermophilus* | *Saccharoplasma lentus* | Carbon byproducts (benefit) |
| E02 | *Saccharoplasma lentus* | *Sulfuribacillus thermophilus* | Sugars (benefit) |
| E03 | *Saccharoplasma lentus* | *Gelatinocella wardii* | Repair fuel (benefit) |
| E08 | *Cryovora brineveil* | *Saccharoplasma lentus* | Trace salts (benefit) |
| E09 | *Saccharoplasma lentus* | *Luminocystis ventor* | Sugars (benefit) |
| E12 | *Mycocladus sulfuris* | *Saccharoplasma lentus* | Recycled carbon (benefit) |
